...
BEGIN
    ...
    FROM dwd_enterprise.dwd_enterprise_enterprise_info_f INTO records,u_last_update_time;

    IF (records = 0)
    THEN

        --创建临时表，简化SQL
        create temp table temp_max_all_dwi_pmp_zhyq_enterprise_main_info
        as
        SELECT
            b.id,
            MAX(b.marking_update) AS marking_update
        FROM
            dwi_pmp.dwi_pmp_zhyq_enterprise_main_info b
        GROUP BY
            b.id;
...
#foreach($filed in $fileds)
        ...
        SELECT a.id,
               ...
               u_current_day
        FROM
            dwi_pmp.dwi_pmp_zhyq_enterprise_main_info a
                JOIN
                    temp_max_all_dwi_pmp_zhyq_enterprise_main_info x
                        ON (
                            a.id = x.id
    ...
    ELSE
        ...
        -- 删除DWR表中当天的数据
        DELETE
        FROM dwd_enterprise.dwd_enterprise_enterprise_info_f
        ...
        SELECT
            ...
            a.dw_batch_number,
            u_current_day
        FROM dwd_enterprise.dwd_enterprise_enterprise_info_f a
        WHERE TO_CHAR((dw_data_partition_date + interval '1 D'), 'yyyy-MM-dd') = u_current_day;

        --查询出dwi表中是否有新增数据,有则增量更新
        SELECT count(1)
        FROM dwi_pmp.dwi_pmp_zhyq_enterprise_main_info
        ...
        THEN
            ...
            WHERE
              ...
              AND record_id in (
                ...
                FROM
                    dwi_pmp.dwi_pmp_zhyq_enterprise_main_info a
                        JOIN dwd_enterprise.dwd_enterprise_enterprise_info_f b ON (a.id = b.record_id)
                WHERE
            ...
            --创建临时表，简化SQL
            create temp table temp_max_current_dwi_pmp_zhyq_enterprise_main_info
            as
            SELECT
                b.id,
                MAX(b.marking_update) AS marking_update
            ...
            FROM
                dwi_pmp.dwi_pmp_zhyq_enterprise_main_info a
                    JOIN
                        temp_max_current_dwi_pmp_zhyq_enterprise_main_info x
                            ON (
                                a.id = x.id
                                AND a.marking_update = x.marking_update
                            )
    ...
    --创建分区
    EXECUTE IMMEDIATE ('call public.sp_add_day_partitions(''dwd_enterprise'',''dwd_enterprise_enterprise_info_f'',''dw_data_partition_date'',''p'',current_partition_name)');
