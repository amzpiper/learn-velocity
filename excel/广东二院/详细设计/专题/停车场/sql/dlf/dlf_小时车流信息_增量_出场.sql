DECLARE
-- gdeyy script_dm_gdeyy_park_car_traffic_h_f_park_out 汇聚主题库：小时车流信息
...
--    源表：  表名：dwi_park_car_traffic_out,   schema : dwi_park,  中文名：车辆入场信息；
--    作者：  huanghh
--    创建日期：  2021-02-20
--    审核人：  huanghh
--    审核时间：  2021-02-20
...
BEGIN
    ...
    WHERE statistical_time >=  u_current_day_start_time  and  statistical_time <=  u_current_day_end_time and entrance_exit_signs = '出场';
    
    --创建临时表，简化SQL
    CREATE temp TABLE temp_all_dwi_park_car_traffic_out
    AS
    select
    ...
    from dwi_park.dwi_park_car_traffic_out
    ...
    SELECT
        ...
        '${job_plan_time}',
        ''
    FROM  temp_all_dwi_park_car_traffic_out a    -- 车辆入场信息
