DECLARE
-- gdeyy script_dm_gdeyy_energy_electric_type_d_f_dm_gdeyy_energy 汇聚主题库：用电数据汇总_设备类型（天）
--    目的：    全量分区 增量采集dwi层 用电数据采集_设备类型（小时） 插入到dwd层中 用电数据汇总_设备类型（天） 表中，同时对数据进行清洗、去重
--    目标表：  表名：dm_gdeyy_energy_electric_type_d_f ,   schema : dm_gdeyy_energy,   中文名：用电数据汇总_设备类型（天）；
--    源表：  表名：dm_gdeyy_energy_electric_type_h_f,   schema : dm_gdeyy_energy,  中文名：用电数据采集_设备类型（小时）；
--    作者：  lixj
--    创建日期：  2021-01-22
--    审核人：  lixj
--    审核时间：  2021-01-22
...
BEGIN
    ...
    -- 支持重跑，删除DM表中近两天的数据
    DELETE FROM dm_gdeyy_energy.dm_gdeyy_energy_electric_type_d_f  -- 用电数据汇总_设备类型（天）
    WHERE acquisition_time >=  u_current_day_start_time and  acquisition_time <=  u_current_day_end_time;

    -- 2.新增数据

    INSERT INTO
        dm_gdeyy_energy.dm_gdeyy_energy_electric_type_d_f (    -- 用电数据汇总_设备类型（天）
        record_id,    --顺序号
        acquisition_time,
        type,
        consumption,
    ...
    SELECT
        ...
        '${job_plan_time}',
        ''
    FROM  dm_gdeyy_energy.dm_gdeyy_energy_electric_type_h_f a    -- 用电数据采集_设备类型（小时）
