DECLARE
-- gdeyy script_dm_gdeyy_energy_water_m_f_dm_gdeyy_energy 汇聚主题库：用水数据汇总（月）
--    目的：    全量分区 增量采集dwi层 用水数据汇总（天） 插入到dwd层中 用水数据汇总（月） 表中，同时对数据进行清洗、去重
--    目标表：  表名：dm_gdeyy_energy_water_m_f ,   schema : dm_gdeyy_energy,   中文名：用水数据汇总（月）；
--    源表：  表名：dm_gdeyy_energy_water_d_f,   schema : dm_gdeyy_energy,  中文名：用水数据汇总（天）；
--    作者：  lixj
--    创建日期：  2021-01-22
--    审核人：  lixj
--    审核时间：  2021-01-22
...
BEGIN
    ...
    -- 支持重跑，删除DM表中近两月的数据
    DELETE FROM dm_gdeyy_energy.dm_gdeyy_energy_water_m_f  -- 用水数据汇总（月）
    WHERE acquisition_time >=  u_current_day_start_time and  acquisition_time <  u_current_day_end_time;

    -- 2.新增数据

    INSERT INTO
        dm_gdeyy_energy.dm_gdeyy_energy_water_m_f (    -- 用水数据汇总（月）
        record_id,    --顺序号
        acquisition_time,
        acquisition_device,
        building,
    ...
    SELECT
        ...
        '${job_plan_time}',
        ''
    FROM  dm_gdeyy_energy.dm_gdeyy_energy_water_d_f a    -- 用水数据汇总（天）
