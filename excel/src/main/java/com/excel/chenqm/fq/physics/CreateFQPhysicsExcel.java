...
public class CreateFQPhysicsExcel {
...
//		titleStyle.setFillForegroundColor(IndexedColors.SEA_GREEN.index);
		...
		try {
			...
			for (Table table : tableList) {
				...
				idRow.createCell(0).setCellValue(CreateFQExcel.dbSchemaPre);
			    ...
			    for (Field field : table.getFieldList()) {
					...
					rr.createCell(0).setCellValue(CreateFQExcel.dbSchemaPre);
					...
					rr.createCell(3).setCellValue(physicsName);
		...
		wbnew.write(new FileOutputStream("d:\\test_fq_physics_db.xlsx"));
		wbnew.close();
