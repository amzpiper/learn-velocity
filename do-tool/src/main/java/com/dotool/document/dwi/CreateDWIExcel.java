...
//	public static String fileName = "d:\\gd2yy_???.xlsx";
...
//	public static String needTableString = "";
	...
	public static void main(String[] args) throws Exception {
		...
		//List<Table> tableList = ReadFQExcel.read(CreateDWIExcel.fileName);
		System.out.println(tableList.size());

		String[] col = new String[] {"??????","???????","??????????","???????","???????","??��???","???????","?????","???????","??/?�՛�","????????????","?????","Schema???","????","????????","?????","?????????","???????","??��???","???????","??????","????"};
...
//		titleStyle.setFillForegroundColor(IndexedColors.SEA_GREEN.index);
		...
		try {
			...
			for (Table table : tableList) {
				...
				String dwiPhysicsTableName = name + "";
				...
				row.createCell(0).setCellFormula("HYPERLINK(\"" + "#???????!A1" + "\",\"" + "????" + "\")");
...
//				}
				...
				for (Field field : table.getFieldList()) {
					...
					rr.createCell(0).setCellValue(DocumentMain.domainNameDwi);
...
//					}
					...
					rr.createCell(2).setCellValue(field.getName());
					...
					rr.createCell(5).setCellValue(lengthStr);
					...
					rr.createCell(11).setCellValue(DocumentMain.sourceSystemNameDwi);
				...
				auditDataList.add("?????????,dw_creation_by,character varying,100");
				auditDataList.add("??????????,dw_creation_date,timestamp,6");
				auditDataList.add("????????????,dw_last_update_by,character varying,100");
				auditDataList.add("?????????????,dw_last_update_date,timestamp,6");
				auditDataList.add("???????��?,dw_batch_number,bigint,");
...
//	public static String fileName = "d:\\gd2yy_�ܺ�.xlsx";
//	public static String domainName = "�ܺ�";
//	public static String sourceSystemName = "�ܺ�";
//	public static String name = "energy";
//	public static String dbSchemaPre = "dwi_" + name;
//	public static String dbPre = "dwd_" + name;
//	public static String dwiDBPre = "";
...
//					}
		...
		wbnew.write(new FileOutputStream("d:\\test�����ܣ�.xlsx"));
		wbnew.close();
		CreateDWIPhysicsExcel.main(args);
	}
